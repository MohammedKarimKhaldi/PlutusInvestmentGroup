    ...
    function staffingLinkForDeal(deal) {
      const params = new URLSearchParams();
      if (deal && deal.id) params.set("dealId", String(deal.id));
      if (deal && deal.name) params.set("deal", String(deal.name));
      if (deal && deal.company) params.set("company", String(deal.company));
      const query = params.toString();
      return `deal-staffing.html${query ? `?${query}` : ""}`;
    }

    function dashboardLinkForDeal(deal) {
      const dashboardId = String((deal && deal.fundraisingDashboardId) || "").trim();
      if (!dashboardId) return "";
      return `investor-dashboard.html?dashboard=${encodeURIComponent(dashboardId)}`;
    }
      ...
      dealsData.forEach(deal => {
        ...
        tr.innerHTML = `
          ...
          <td>
            ...
            &nbsp;|&nbsp;
            <a class="action-link" href="${staffingLinkForDeal(deal)}">Staffing</a>
            ${dashboardLinkForDeal(deal)
              ? `&nbsp;|&nbsp;<a class="action-link" href="${dashboardLinkForDeal(deal)}">Dashboard</a>`
              : ""}
          </td>
        `;
