...
function renderDealHeader() {
  ...
  const sidebarDashboardLink = document.querySelector('.sidebar-nav .nav-link[href="investor-dashboard.html"]');
  if (sidebarDashboardLink) {
    const dashId = String(currentDeal.fundraisingDashboardId || "").trim();
    sidebarDashboardLink.setAttribute(
      "href",
      dashId ? `investor-dashboard.html?dashboard=${encodeURIComponent(dashId)}` : "investor-dashboard.html",
    );
  }

  const btnStaffing = document.getElementById("btn-open-staffing");
  if (btnStaffing) {
    btnStaffing.onclick = () => {
      const params = new URLSearchParams();
      params.set("dealId", currentDeal.id || "");
      if (currentDeal.name) params.set("deal", currentDeal.name);
      if (currentDeal.company) params.set("company", currentDeal.company);
      window.location.href = `deal-staffing.html?${params.toString()}`;
    };
  }
